...
*   **Custom Specialized Agents:** Users can dynamically define and inject custom agents (e.g., Salesforce expert) using the standard `agency-agents` format. Features a multi-step creator wizard.
...
*   **Micro-Tasking via Chat:** Ability to bypass full project scoping for quick tasks via a universal command interface.
