...
1. **Entry:** User navigates to the Agent Registry and clicks "Create Custom Agent".
2. **Definition:** User defines the role ("HubSpot CRM Expert"), backstory, and specific tools required.
3. **Export/Save:** The system saves the agent using the standardized `agency-agents` format, ensuring it is instantly recognizable by the central orchestrator.
4. **Utilization:** The user creates a new task "Configure HubSpot pipelines" and the Orchestrator successfully assigns it to the newly created custom agent.