...
*   **Fluid Navigation Paradigm:** Moving away from distinct page reloads, the app will adopt a single-page application (SPA) feel with a persistent collapsible left sidebar. Global navigation, recent workspaces, and a **"Workspace Tools"** section (for explicit tool management like Agents and Files, mimicking Gemini's "Gems") become accessible overlays rather than distinct URL destinations.
    *   **RBAC Architecture:** The Hybrid Sidebar renders conditionally. `Agency Admin` sees all tools and admin utilities. `Agency Staff` sees only execution tools. `Client` roles see a severely restricted sidebar (Dashboard and Files only).
