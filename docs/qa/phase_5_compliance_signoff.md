...
- **Required Action:** Ensure that OpenTelemetry (OTel) traces and Promtail logs implement automated PII scrubbing (e.g., masking user IPs, emails, or sensitive query parameters) before log ingestion. (Added to technical design)
...
- **Required Action:** Implement a strict cookie consent barrier. GTM scripts (GA4, PostHog, Meta, Twitter) must be blocked by default and only activated upon explicit opt-in (GDPR) or provide clear opt-out mechanisms with respect to "Do Not Sell" signals (CCPA). (Added CMP requirement to technical design)

## 3. Legal Review Sign-Off
**Sign-Off Status:** APPROVED
**Reason:** The technical design has been updated to include explicit architectural provisions for a cookie consent management platform (CMP) barrier before the activation of GTM tracking pixels, satisfying GDPR and CCPA requirements. Automated PII scrubbing for Telemetry (OpenTelemetry/Promtail) has also been mandated.

**Next Steps:** Proceed with Phase 5 implementation (Epic 5.1).
